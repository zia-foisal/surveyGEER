...
This utilizes the [MODIS Terra Vegetation Indices 16-Day Global 250 m ](https://developers.google.com/earth-engine/datasets/catalog/MODIS_061_MOD13Q1)[@funk2015chirps] data set to create a variety of drought indicators. They indicators are then aggregated to hexagon grid which is output for QGIS map automation and visualization through the `Atlas` functionality.
...
bgd_adm0_dissolved <- st_read(con,"bgd_adm1") |> 
  summarise() |> 
  mutate(uid=1)

atlas_victim <- bgd_adm0_dissolved |> 
  left_join(
  grid_with_ndvi_yrmo |> 
  mutate(yr_mo = paste0(lubridate::month(date,label=T,abbr = F)," ",lubridate::year(date))) |> 
  distinct(date, yr_mo) |> 
...
atlas_victim|> 
  st_write(
    "E:\\Synology\\BGD Drought Monitoring\\bgd_hex_climate_maps_v1.gpkg",
    "bgd_atlas_coverage",delete_layer=T, overwrite=T
  )
